...
class Solution {
    public int robotSim(int[] commands, int[][] obstacles) {
        ...
        for (int[] obstacle : obstacles) {
            set.add(calHash(obstacle[0], obstacle[1]));
        }

        int dir = 0, x = 0, y = 0; //北:0 东:1 南:2 西:3
        int ans = 0;
        int[] dx = {0, 1, 0, -1};
        int[] dy = {1, 0, -1, 0};
        for (int command : commands) {
            if (command == -2) {
                ...
                for (int i = 0 ; i < command ; i++) {
                    int nx = x + dx[dir];
                    int ny = y + dy[dir];
                    if (set.contains(calHash(nx, ny))) break;
                    x = nx;
                    y = ny;
